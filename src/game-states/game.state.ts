...
export class GameState implements State {
  ...
  constructor() {
    ...
    this.player = new FirstPersonPlayer(new Camera(Math.PI / 3, aspect, 1, 500), AiNavPoints[0]);
    ...
    window.addEventListener('resize', () => {
      if (this.player?.camera) {
        const canvas = document.getElementById('c3d') as HTMLCanvasElement;
        const aspect = canvas.clientWidth / canvas.clientHeight;
        this.player.camera.projection = this.createProjectionMatrix(Math.PI / 3, aspect, 1, 500);
      }
    });
  }

  private createProjectionMatrix(fieldOfViewRadians: number, aspect: number, near: number, far: number): DOMMatrix {
  ...
  onUpdate() {
          ...
          if (distance < 1 || direction < -0.77) {
            if (door.isLocked) {
              if (this.player.heldKeyRoomNumber === this.player.closestNavPoint.roomNumber) {
                tmpl.innerHTML += `<div style="font-size: 30px; text-align: center; position: absolute; bottom: 20px; width: 100%;">🗝️ &nbsp; 解锁并开门</div>`;
                ...
                tmpl.innerHTML += `<div style="font-size: 30px; text-align: center; position: absolute; bottom: 20px; width: 100%;">🔒 &nbsp; 已上锁</div>`;
              }
              ...
              tmpl.innerHTML += `<div style="font-size: 30px; text-align: center; position: absolute; bottom: 20px; width: 100%;">${door.openClose === -1 ? '开门' : '关门'}</div>`;
              if (controls.isConfirm) {
                door.pullLever();
              }
            }
      ...
      if (hidingPlace) {
        ...
        if (distance < 8) {
          ...
          if (direction < -0.77 && !this.player.isHiding) {
            tmpl.innerHTML += `<div style="font-size: 30px; text-align: center; position: absolute; bottom: 20px; width: 100%;">躲藏</div>`;
          ...
          if (direction < -0.9) {
            if (item.roomNumber) {
              if (item.roomNumber === -1) {
                tmpl.innerHTML += `<div style="font-size: 30px; text-align: center; position: absolute; bottom: 20px; width: 100%;">🎂 许个愿</div>`;
              } else {
                tmpl.innerHTML += `<div style="font-size: 30px; text-align: center; position: absolute; bottom: 20px; width: 100%;">🗝️ 拾取 ${item.roomNumber} 号房间钥匙</div>`;
              }
            } else {
              tmpl.innerHTML += `<div style="font-size: 30px; text-align: center; position: absolute; bottom: 20px; width: 100%;">使用医疗包</div>`;
            }
            if (controls.isConfirm && !controls.prevConfirm) {
              item.isTaken = true;
              if (item.roomNumber !== -1) {
    ...
    if (this.hasPlayerLeftElevator && this.player.feetCenter.z < 3) {
      ...
      tmpl.innerHTML += `<div style="font-size: 40px; text-align: center; position: absolute; bottom: 20px; width: 100%;">你赢了！</div>`;
      if (!this.isGameEnded) {
